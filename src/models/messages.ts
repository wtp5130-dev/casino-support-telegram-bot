...
export async function getMessagesForConversation(conversation_id: number) {
  const res = await sql`SELECT * FROM messages WHERE conversation_id=${conversation_id} ORDER BY created_at ASC, id ASC`;
  return (res || []) as any;
}

export async function findInboundByTelegramId(conversation_id: number, telegram_message_id: string) {
  const res = await sql`SELECT * FROM messages WHERE conversation_id=${conversation_id} AND direction='in' AND telegram_message_id=${telegram_message_id} LIMIT 1`;
