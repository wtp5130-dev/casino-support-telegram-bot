...
import { findInboundByTelegramId, getMessagesForConversation, insertMessage } from './models/messages.js';
...
app.post('/telegram/webhook', async (req, res) => {
  ...
  try {
    ...
    if (text === '/start') {
      ...
      insertMessage({ conversation_id: convo.id, direction: 'out', role: 'assistant', text: welcome, telegram_message_id: String(outId || '') });
      return res.json({ ok: true });
    }
    if (text === '/help') {
      ...
      insertMessage({ conversation_id: convo.id, direction: 'out', role: 'assistant', text: help, telegram_message_id: String(outId || '') });
      return res.json({ ok: true });
    }
